krs_in_regio1 = {
    'Nordrhein_Westfalen': ['Aachen', 'Bielefeld', 'Aachen_Kreis', 'Euskirchen_Kreis', 'Steinfurt_Kreis', 'Soest_Kreis',
    ...
    'Rheinland_Pfalz': ['Kusel_Kreis', 'Vulkaneifel_Kreis', 'Donnersbergkreis', 'Neuwied_Kreis', 'Pirmasens',
    ...
    'Bayern': ['Kelheim_Kreis', 'Traunstein_Kreis', 'Regen_Kreis', 'Augsburg_Kreis', 'Aschaffenburg_Kreis',
    ...
    'Baden_Württemberg': ['Heilbronn_Kreis', 'Ludwigsburg_Kreis', 'Esslingen_Kreis', 'Zollernalbkreis',
    ...
    'Thüringen': ['Greiz_Kreis', 'Nordhausen_Kreis', 'Gotha_Kreis', 'Ilm_Kreis', 'Wartburgkreis',
    ...
    'Sachsen_Anhalt': ['Wittenberg_Kreis', 'Harz_Kreis', 'Salzlandkreis', 'Stendal_Kreis', 'Börde_Kreis',
    ...
    'Niedersachsen': ['Wesermarsch_Kreis', 'Harburg_Kreis', 'Hannover_Kreis', 'Emsland_Kreis',
                      ...
                      'Vechta_Kreis'],
    'Brandenburg': ['Märkisch_Oderland_Kreis', 'Oberhavel_Kreis', 'Havelland_Kreis', 'Cottbus',
    ...
    'Sachsen': ['Leipzig_Kreis', 'Mittelsachsen_Kreis', 'Leipzig', 'Erzgebirgskreis', 'Nordsachsen_Kreis',
    ...
    'Hessen': ['Hochtaunuskreis', 'Kassel_Kreis', 'Wetteraukreis', 'Offenbach_Kreis', 'Vogelsbergkreis',
    ...
    'Schleswig_Holstein': ['Nordfriesland_Kreis', 'Segeberg_Kreis', 'Dithmarschen_Kreis',
    ...
    'Mecklenburg_Vorpommern': ['Parchim_Kreis', 'Ludwigslust_Kreis', 'Nordwestmecklenburg_Kreis',
    ...
    'Saarland': ['Stadtverband_Saarbrücken_Kreis', 'Sankt_Wendel_Kreis', 'Saarpfalz_Kreis', 'Saarlouis_Kreis',
...
if __name__ == "__main__":
    from demonstration.demonstration_immo_data import UNORDERED_CATEGORICAL_VALUES
    import pandas as pd
    import os
    import string
    ...
    raw_data = pd.read_csv(os.path.join(PATH, "landkreise.csv"), sep=";", names=["kreis", "land"])

    print(raw_data.head())

    laender = list(raw_data["land"])
    kreise = list(raw_data["kreis"])

    all_values = []
    for krse in krs_in_regio1.values():
        all_values += [krs for krs in krse]

    print("Sorted:", len(all_values))
    print("Total:", len(UNORDERED_CATEGORICAL_VALUES["geo_krs"]))
    for krs in UNORDERED_CATEGORICAL_VALUES["geo_krs"]:
        if krs not in all_values:
            print(krs)

        if all_values.count(krs) > 1:
            print("doppel", krs)

    # record = {key: [] for key in raw_data["land"].unique()}
    #
    # for krs in UNORDERED_CATEGORICAL_VALUES["geo_krs"]:
    #     if krs in all_values:
    #         continue
    #
    #     search_krs = krs.replace("_Kreis", "").replace("_", " ")
    #     search_krs = "".join((s for s in search_krs if s in string.ascii_letters + " "))
    #     print(krs, search_krs)
    #
    #     region = "huhuhu"
    #     while f"{region} {region}" not in record:
    #         region = input("?")
    #     if region:
    #         record[f"{region} {region}"].append(krs)
    #
    #     # for i, kreis in enumerate(kreise):
    #     #     kreis = "".join((s for s in kreis if s in string.ascii_letters))
    #     #     if search_krs in kreis:
    #     #         if search_krs == kreis or not input(f"{krs} -> {kreis}?"):
    #     #             record[laender[i]].append(krs)
    #     #             break
    #
    # print(record)
