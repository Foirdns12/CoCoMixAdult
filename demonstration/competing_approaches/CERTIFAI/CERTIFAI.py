...
class CERTIFAI:
    ...
    def set_constraints(self, x=None, fixed=None):
        ...
        if isinstance(x, pd.DataFrame):
            ...
            x_tmp = pd.DataFrame(x).apply(try_conversion)
            num = x_tmp.select_dtypes(include="number").columns
            ...
            for i in range(x.shape[1]):
                if i in fixed_feats:
                    ...
                    self.constraints.append((0, len(np.unique(x[:, i]))))
    ...
    def generate_candidates_tab(self,
        ...
        if has_cat:
            ...
            generation = generation.astype(sample.dtype)
            ...
            for idx, ncat, cat_value in cat_ids:
                ...
                generation[:, idx] = random_cats

            distances = self.distance(sample, generation)
        ...
        return generation.tolist(), distances
    ...
    def fit(self,
        ...
        has_cat = len(cat_ids) > 0

        if classification and class_specific is not None:
            x = x[self.predictions == class_specific]
        ...
        for i in tot_samples:
            ...
            for g in range(generations):
                ...
                counter_preds = self.generate_prediction(model,
                                                         ...
                                                         pytorch,
                                                         classification)
                if self.predictions[0] < 6:
                    diff_prediction = [counter_pred == self.predictions[0]+2 for
                ...
                #diff_prediction = [counter_pred != self.predictions[0]  for
                 #                      counter_pred in counter_preds]
                if sum(bool(x) for x in diff_prediction)==0:
                    continue
                final_generation = crossed_generation.loc[diff_prediction]

                final_distances = self.distance(sample, final_generation)
                #fitnesstest.append(sum(final_distances) / len(final_distances))
                ...
                assert len(counterfacts) == len(
                    counterfacts_fit), 'Something went wrong: len(counterfacts): {}, len(counterfacts_fit): {}'.format(
                    len(counterfacts), len(counterfacts_fit))
                ...
                if len(current_samples) > 1:
                    current_expect_distance = np.array(
                        [self.distance(
                            ...
                            current_samples) for i in range(
