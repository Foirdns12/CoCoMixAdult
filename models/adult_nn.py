...
from data.adult import load_df, CATEGORICAL
...
def df_to_dataset(dataframe, shuffle=True, batch_size=32, target="label"):
...
def train_model(target="label"):
    ...
    model.save(os.path.join(PATH, "adult_nn.tf"), save_format="tf")
...
def load_model():
    return tf.keras.models.load_model(os.path.join(PATH, "adult_nn.tf"))
...
def inspect_confusion_matrix():
    ...
    df, target, test, train, val = prepare_data("label")
