...
from demonstration.demonstration_adult_data import load_df, CATEGORICAL, RANDOM_STATE, fill_numerical_column_by_cond_median, \
    FEATURES, NUMERICAL
...
def prepare_data(drop_target=True):
    ...
    if False:#os.path.exists(data_path):
        ...
        if True:
            ...
            df_train["target"], bins = pd.qcut(df_train[TARGET], 2, labels=False, retbins=True)
        if drop_target:
...
def df_to_dataset(dataframe, shuffle=True, batch_size=64, target="label"):
...
def train_model(layer_conf=None):
    ...
    model.save(os.path.join(PATH, f"{current_time}adult_nn.tf"), save_format="tf")
    model.save_weights(os.path.join(PATH, f"{current_time}adult_nn.weights"))
...
def load_model(code="20201214-133037"):
    ...
    model = tf.keras.models.load_model(os.path.join(PATH, f"{code}adult_nn.tf"),
...
def load_model_from_weights(code="20201214-133037", layer_conf=((0.0, 256), (0.1, 256))):
    ...
    model.load_weights(os.path.join(PATH, f"{code}adult_nn.weights"))
    return model
